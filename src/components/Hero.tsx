...
const Hero = () => {
  ...
  return (
    <section className="relative pt-24 pb-16 md:pt-28 md:pb-24 overflow-hidden">
      ...
      <div className="absolute top-0 right-0 w-1/2 h-1/2 bg-sunny-yellow/20 rounded-full blur-3xl -translate-x-1/2 -translate-y-1/4 z-0"></div>
      <div className="absolute bottom-0 left-0 w-1/2 h-1/2 bg-sunny-orange/20 rounded-full blur-3xl translate-x-1/4 translate-y-1/4 z-0"></div>
      
      {/* Removed className="inner" from here as it's now in Index.tsx */}
      <div className="container px-4 md:px-6 relative z-10"> {/* Using container for consistent padding with .inner */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12 items-center">
          <div className="flex flex-col gap-6 text-center md:text-left">
            ...
            {/* Popular Destinations & Mini Sign Up Form alignment */}
            <div className="mt-4 grid sm:grid-cols-2 gap-6 items-end"> {/* Changed gap-4 to gap-6 and items-center to items-end for better baseline align */}
              <div className="bg-white/70 backdrop-blur-sm p-3 rounded-2xl shadow-sm hidden md:block">
              ...
              {/* Mini signup form placed here in the grid */}
              <div className="w-full"> {/* Ensure form takes full width of its grid cell */}
                 <MiniSignUpForm />
              </div>
            </div>
          </div>
        ...
        {/* Bottom decorative elements - No MiniSignUpForm here anymore */}
        <div className="mt-8 flex justify-center"> {/* Increased margin top for spacing */}
