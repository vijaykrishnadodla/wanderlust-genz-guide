...
// import { Link } from 'react-router-dom'; // No longer needed for logo
...
const Navbar = () => {
  ...
  const handleLogoClick = (event: React.MouseEvent<HTMLAnchorElement>) => {
    ...
    // Close mobile menu if it's open, though less likely to be clicked from expanded menu
    if (isMenuOpen) {
        setIsMenuOpen(false);
    }
  };
      ...
      <div className="inner">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="hidden md:flex items-center space-x-3">
            <a href="#sunshine-club" onClick={(e) => {
                e.preventDefault();
                const section = document.querySelector('#sunshine-club');
                if (section) {
                    section.scrollIntoView({ behavior: 'smooth' });
                }
                setIsMenuOpen(false); // Ensure menu closes if this was somehow triggered from mobile context
            }}>
