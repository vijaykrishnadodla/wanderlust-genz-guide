      ...
      <div className="container px-4 md:px-6 mx-auto">
        <div className="flex items-center justify-between h-16">
          ...
          <nav className="hidden md:flex items-center space-x-1">
            {['Home', 'Sales Page', 'Funnel', 'Memberships', 'Quiz'].map((item, index) => (
              <Link 
                key={index}
                to={item === 'Home' ? '/' : item === 'Sales Page' ? '/sales' : item === 'Funnel' ? '/funnel' : `#${item.toLowerCase()}`}
          ...
          <div className="hidden md:flex items-center space-x-3">
            <Button
              variant="outline"
              className="rounded-full border-sunny-orange text-sunny-orange hover:bg-sunny-orange/10"
            >
          ...
          <div className="container px-4 py-4">
            <nav className="flex flex-col space-y-2">
              ...
              <div className="pt-3 flex flex-col space-y-2 border-t border-sunny-yellow-light/30 mt-2">
                <Button
                  variant="outline"
                  className="w-full justify-center rounded-full border-sunny-orange text-sunny-orange hover:bg-sunny-orange/10"
                >
                  <Sun className="mr-2 h-4 w-4" /> Sign In
                </Button>
